...
$(()=>{
	...
	autoSave = new AutoSave('auto_save', crudBaseData.csrf_token);
	...
	rowExchange = new RowExchange('main_tbl', data, null, (param)=>{
		...
		let auto_save_url = 'mission/auto_save';
	...
	pwms = new ProcessWithMultiSelection({
			...
			csrf_token:crudBaseData.csrf_token,
	});
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'mission/destroy'; // Ajax通信先URL
...
function regAction(){
	...
	// SPA型・登録アクション
	let ajax_url = crudBaseData.paths.public_url + '/mission/reg_action';
